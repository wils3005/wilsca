...
import { WebSocketServerWrapper } from "../src/web-socket-server-wrapper";
...
const createTestServer = ({
  ...
  url: string;
}): Destroyable<WebSocketServerWrapper> => {
  const server = new Server(url);
  const webSocketServerWrapper: Destroyable<WebSocketServerWrapper> = new WebSocketServerWrapper(
    {
      server,
      realm,
      config,
  ...
  server.on(
    ...
    (
      ...
      const s = webSocketServerWrapper.webSocketServer;
  ...
  webSocketServerWrapper.destroy = async (): Promise<void> => {
    server.close();
  };
...
describe("WebSocketServerWrapper", () => {
  it("should return valid path", () => {
    ...
    const webSocketServerWrapper = new WebSocketServerWrapper({
      server,
      realm,
      config,
    });

    expect(webSocketServerWrapper.path).to.be.eq("/peerjs");

    const webSocketServerWrapper2 = new WebSocketServerWrapper({
      server: server2,
      realm,
      config: config2,
    });

    expect(webSocketServerWrapper2.path).to.be.eq("path/peerjs");
  ...
  it(`should check client's params`, async () => {
    ...
    const getError = async (
      ...
      const WebSocketServerWrapper = createTestServer({ url, realm, config });
      ...
      await WebSocketServerWrapper.destroy?.();
  ...
  it(`should check concurrent limit`, async () => {
    ...
    const createClient = (id: string): Destroyable<WebSocket> => {
      ...
      ws.destroy = async (): Promise<void> => {
        ...
        WebSocketServerWrapper.destroy?.();
